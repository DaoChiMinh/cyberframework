...
class UploadObject {
  ...
  static UploadSourceType _detectSourceType(dynamic source) {
    ...
    // Check bytes
    if (source is List<int> || source.runtimeType.toString().contains('Uint8List')) {
  ...
  Future<String> _downloadAndEncode(String url) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Không thể download file từ URL: ${response.statusCode}');
      }
  ...
  String? _detectExtensionFromBase64(String base64String) {
    try {
      ...
      final bytes = base64.decode(base64String.substring(0, min(100, base64String.length)));
  ...
  String get sourceTypeName {
    switch (sourceType) {
      case UploadSourceType.filePath: return 'File Path';
      case UploadSourceType.url: return 'URL';
      case UploadSourceType.base64: return 'Base64';
      case UploadSourceType.fileObject: return 'File Object';
      case UploadSourceType.bytes: return 'Bytes';
      case UploadSourceType.xfile: return 'XFile';
      case UploadSourceType.unknown: return 'Unknown';
    }
  }
